...
**Definició:** selector CSS que aplica estils a la primera lletra d'un element de bloc.
**Explicació:** S'usa per crear lletres capitals decoratives (drop caps) al començament d'un paràgraf o article. Requereix que l'element tingui `display: block` o similar. Es combina amb `float: left` perquè el text l'emboliqui.
**Exemple:**
```css
...
## `linear-gradient`
**Definició:** funció CSS que crea un degradat lineal entre dos o més colors.
**Explicació:** Es pot usar per crear patrons geomètrics amb múltiples gradients superposats. Combinant `linear-gradient` amb `background-size` i `background-position` es poden fer mosaics (com l'opus tessellatum romà).
**Exemple:**
```css
.hero::before {
  background-image:
    linear-gradient(45deg, #C4953C 25%, transparent 25%),
    linear-gradient(-45deg, #C4953C 25%, transparent 25%);
...
**Explicació:** Alternativa a `<link>` per a fonts o CSS extern. Es posa al principi del full. S'usa per carregar Google Fonts directament des del CSS.

## `clamp()`
**Definició:** funció CSS que estableix un valor entre un mínim i un màxim, ajustant-se proporcionalment.
**Explicació:** Ideal per a mides de font responsives. `font-size: clamp(2rem, 5vw, 3.5rem)` vol dir: comença a 2rem, creix fins a 3.5rem segons l'amplada de pantalla. Estalvia media queries.
**Exemple:**
```css
...
**Definició:** sufix de Tailwind que estableix l'opacitat d'un color amb un sol valor.
**Explicació:** En lloc d'usar `rgba()`, Tailwind permet escriure `text-pedra-clara/70` per obtenir el color amb opacitat 70%. El nombre va de 0 a 100.
**Exemple:**
```html
<p class="text-pedra-clara/70">Text semitransparent</p>
```

## Tailwind `grid-cols-[...]` (arbitrary values)
**Definició:** sintaxi de Tailwind per definir columnes de grid amb valors personalitzats entre claudàtors.
**Explicació:** Quan les classes predefinides (com `grid-cols-3`) no són suficients, es pot usar qualsevol valor CSS vàlid. Molt útil per a `grid-template-columns: 1fr 280px`.
**Exemple:**
```html
<div class="grid grid-cols-[1fr_280px] gap-10">
  <main>Principal</main>
  <aside>Sidebar</aside>
</div>
```
...
## Google Fonts
**Definició:** servei de fonts web gratuït de Google que ofereix tipografies d'alta qualitat.
**Explicació:** Es carreguen via `@import` al CSS o `<link>` al HTML. S'especifica la família, pesos i estils necessaris per no carregar de més. Es poden usar amb Tailwind configurant `fontFamily` al tema.
**Exemple:**
```css
...
**Explicació:** En comptes d'escriure CSS personalitzat, combines classes predefinides com `text-center`, `bg-blue-500` o `p-4` directament al HTML. Accelera molt el desenvolupament. Es pot carregar via CDN amb `<script src="https://cdn.tailwindcss.com"></script>`.
**Exemple:**
```html
...
## Tailwind Config (CDN)
**Definició:** configuració personalitzada de Tailwind CSS directament al HTML.
**Explicació:** Amb la CDN es pot configurar el tema (colors, fonts) mitjançant `tailwind.config = { theme: { extend: {...} } }` dins d'un `<script>` després del CDN per generar classes utilitàries personalitzades.
**Exemple:**
```html
<script>
tailwind.config = {
  theme: {
    extend: {
      colors: { slate: '#0F1923' },
      fontFamily: { cinzel: ['Cinzel', 'Georgia', 'serif'] },
    }
  }
}
</script>
